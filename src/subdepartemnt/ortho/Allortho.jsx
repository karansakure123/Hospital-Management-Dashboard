...
const Allortho = () => {
  ...
  useEffect(() => {
    const fetchOrthoServices = async () => {
      try {
        const response = await axios.get('https://hospital-management-backend-4.onrender.com/api/v1/orthopedics/getall');
      ...
      <div className={`toast-confirm ${t.visible ? "animate-enter" : "animate-leave"}`}>
        ...
        <div className="button-group">
          <button
            onClick={async () => {
              try {
                await axios.delete(`https://hospital-management-backend-4.onrender.com/api/v1/orthopedics/delete/${id}`);
